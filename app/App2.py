...
def ws_delete(n_clicks, ndxs, data, tmpdir):
...
def res_heatmap(n_clicks, wdir):
    fn = get_results_fn(wdir)
    mint = Mint()
    mint.load(fn)
    fig = plot_heatmap(mint.crosstab('peak_max'), call_show=False, name='peak_max')
    return fig
