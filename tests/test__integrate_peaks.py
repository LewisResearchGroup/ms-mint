...
from ms_mint.tools import integrate_peaks_from_df
...
def test__integrate_peaks_from_df():
    ...
    result = integrate_peaks_from_df(df, mz_mean, mz_width, rt_min, rt_max, intensity_threshold, peak_label).loc[0, 'peak_area']
    expected = 10
    assert result == expected, f'{result} != {expected}'


def test__integrate_peaks_from_df__rtmax():
    df = fake_ms_data(0, 10, 0, 200, 1, 1)
    mz_mean = 100
    mz_width = 1
    rt_min = 0
    ...
    peak_label = 'Label'
    intensity_threshold = 0
    result = integrate_peaks_from_df(df, mz_mean, mz_width, rt_min, rt_max, 
                                intensity_threshold, peak_label).loc[0, 'peak_area']
    expected = 5
    assert result == expected, f'{result} != {expected}'


def test__integrate_peaks_from_df__rtmin():
    df = fake_ms_data(0, 10, 0, 200, 1, 1)
    mz_mean = 100
    mz_width = 1
    rt_min = 5.1
...
def test__integrate_peaks_from_df__mz():
    df = fake_ms_data(0, 10, 0, 200, 1, 1)
    mz_mean = 100
    rt_min = 0
    rt_max = 10
...
def test__integrate_peaks_from_df__intensity_threshold():
