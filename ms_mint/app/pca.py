...
_layout = html.Div([
    ...
    dcc.Dropdown(id='pca-options', options=options, 
    ...
    dcc.Loading( html.Div(id='pca-figure-explained-variance', style={'margin': 'auto', 'text-align': 'center'}) ),
    html.H4('Scatter plot of principal components'),   
    dcc.Loading( html.Div(id='pca-figure-pairplot', style={'margin': 'auto', 'text-align': 'center'}) ),
...
def callbacks(app, fsc, cache):
    ...
    def create_pca( n_clicks, n_components, facet_height, groupby, 
                    include_labels, exclude_labels, norm_cols, 
                    file_types, options, wdir ):
        if n_clicks is None:
            raise PreventUpdate
        ...
        fig_contrib = px.bar(data_frame=contrib, 
                             x='peak_label', 
                             y='Coefficient', 
                             facet_col='PC', 
                             facet_col_wrap=1, 
